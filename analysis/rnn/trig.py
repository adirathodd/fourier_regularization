"""RNN trigonometric identity analysis entrypoints."""
...
def verify_trigonometric_identity(
    ...
    return common_trig.verify_trigonometric_identity(
        model,
        checkpoint,
        dataset,
...
def verify_trigonometric_identity_subtraction(
    ...
    if not hasattr(model, 'rnn'):
        raise ValueError('RNN analysis requested but model has no rnn module.')
    return common_trig.verify_trigonometric_identity_subtraction(
        model,
        checkpoint,
        dataset,
        freq_threshold_top_fraction=freq_threshold_top_fraction,
